...
Daily aggregator of South Carolina-located taildragger classified
listings, pulled from every one of the taildraggers.com per-manufacturer
scraper repos, published as a static page (`docs/index.html`) meant to
be embedded via `<iframe>` on taildraggers.com's state-listing pages.
...
repos' approach, with the state filter swapped to "SC".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, SC` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in South Carolina"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/south-carolina/`
...
```html
<iframe
  src="https://taildraggers.github.io/south-carolina/"
  title="Taildragger Ads in South Carolina"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_SOUTH_CAROLINA_LOCATION_RE` in `main.py`, and
  updating `PAGE_TITLE`.
