"""Aggregates South Carolina-located listings from every taildraggers.com
...
ones whose location ends in ", SC".
...
Pennsylvania/Rhode Island repos' approach, with the state filter swapped
to "SC".
"""
...
PAGE_TITLE = "Taildragger Ads in South Carolina"
STATE_ABBR = "SC"
...
_SOUTH_CAROLINA_LOCATION_RE = re.compile(r",\s*SC\s*$", re.IGNORECASE)


def _is_south_carolina(location: str) -> bool:
    return bool(_SOUTH_CAROLINA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        south_carolina_rows = [row for row in rows if _is_south_carolina(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(south_carolina_rows)} in South Carolina")
        all_listings.extend(south_carolina_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No South Carolina listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique South Carolina listings: {len(listings)}")
